...
QGC_LOGGING_CATEGORY(TelemetryLogManagerLog, "TelemetryLogManagerLog")
...
{
    connect(qgcApp()->toolbox()->mavlinkProtocol(), &MAVLinkProtocol::manualCaptureActiveChanged,
...
{
    return qgcApp()->toolbox()->mavlinkProtocol()->manualCaptureActive();
}
...
void TelemetryLogManager::refresh()
{
    ...
    const QString savePath = qgcApp()->toolbox()->settingsManager()->appSettings()->telemetrySavePath();
    if (savePath.isEmpty()) {
    ...
    for (const QFileInfo &fileInfo : fileInfoList) {
        ...
        connect(entry, &TelemetryLogEntry::selectedChanged, this, &TelemetryLogManager::_updateSelectedCount);
...
{
    qgcApp()->toolbox()->mavlinkProtocol()->startManualCapture();
}

void TelemetryLogManager::stopAndSaveCapture()
{
    qgcApp()->toolbox()->mavlinkProtocol()->stopManualCapture();
    refresh();
}
